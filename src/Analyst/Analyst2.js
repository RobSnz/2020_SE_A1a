...
class Analyst2 extends React.Component {
  ...
  updateDetails = (article) => {
    this.setState({
      modelInformation: {
        ...
        pagesNum: article.pagesNum,
        numOfPages: article.numOfPages,
        year: article.year,
        month: article.month,
        ePrint: article.ePrint,
        ePrintType: article.ePrintType,
        ePrintClass: article.ePrintType,
  ...
  removeKeyValueHandler = (id) => {
    ...
    this.state.keyValues = this.state.keyValues.filter(
      (item) => item.id !== id
    );
  ...
  getArticleResult = () => {
    axios
      .post("/article/retrieve/toAnalyse")
      .then((response) => {
      ...
      <div className={styles.analystQueue}>
        ...
        <Modal show={this.state.show} onHide={this.handleClose}>
          <Modal.Header closeButton>
            <Modal.Title>{this.state.modelInformation.title}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <Container>
              <Row>
              ...
              <Form.Row>
                ...
                <Form.Group as={Col} controlId="formGridOperator">
                  <Form.Control
                    ...
                    onChange={this.selectOperatorHandler}
                  >
                ...
                <Form.Group as={Col} controlId="formGridValue">
                  <InputGroup className="mb-3">
                    ...
                    <InputGroup.Append>
                      <IconButton
                        onClick={this.addKeyValueHandler}
                        aria-label="add"
            ...
            <ListGroup>
              {this.state.keyValues.map((thing) => (
                ...
                >
                  <ListGroup.Item>
                    {thing.field} {thing.operator} {thing.value}
                  </ListGroup.Item>
                  <InputGroup.Append>
                    <IconButton
                      onClick={() => this.removeKeyValueHandler(thing.id)}
                      aria-label="remove"
                      style={{ float: "right", margin: "-5px 0 0 0" }}
                    >
                      <RemoveIcon />
          ...
          <Modal.Footer>
            <Button
              variant="dark"
              onClick={(e) => this.handleSubmit(e, "accepted")}
            >
              Accept
            </Button>
            <Button
              variant="dark"
              onClick={(e) => this.handleSubmit(e, "rejected")}
            >
              Reject
            </Button>
          </Modal.Footer>
